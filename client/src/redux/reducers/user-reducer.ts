...
export const getRecoveryToken = (email:string): ThunkType => async (dispatch) => {
    try {
        dispatch(userActions.setLoading(true))
...
export const resetPassword = (password:string, token:string): ThunkType => async (dispatch) => {
    try {
        dispatch(userActions.setLoading(true))
...
type ActionsType = InferActionsTypes<typeof userActions | typeof appActions | typeof authActions>
